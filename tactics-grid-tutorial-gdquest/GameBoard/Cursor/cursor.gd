# Player-controller cursor. Allows them to navigate to the game grid, select units, and move them.
...
# We'll use signals to keep thecursor decoupled from other nodes.
# When the player moves the cursor or wants to interact with a cell, we emit a singal and let
...
func _unhandled_imput(event: InputEvent) -> void:
	...
	# If the player is pressing the key in this grame, we allow the cursor to move. If they keep the
...
# We use the draw callback to a rectangular outline the size of a grid cell, with a wifdht of two
# pixels.
func _draw() -> void:
