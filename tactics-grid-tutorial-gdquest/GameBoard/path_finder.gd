...
# Initiazlizes the AStar2D object upon creation.
func _init(grid: Grid, walkable_cells: Array) -> void:
	...
	# needs to initialize itself via its contructor function, _init().
	_grid = grid
...
func calculate_point_path(start: Vector2, end: Vector2) -> PackedVector2Array:
	# Wioth the AStar algorithm, we have to use the points' indices to get a path. This is why we
	...
	if _astar.has_point(start_index) and _astar.has_point(end_index):
		# The AStar2D ovject then finds the best path betwen the two indices.
...
func _add_and_connect_points(cell_mappings: Dictionary) -> void:
	...
	# We pass each cell's unique index and the correspoinding Vector2 coordinates to the
	# AStar2D.add_point() function.
	for point in cell_mappings:
		_astar.add_point(cell_mappings[point],  point)
	
	# Then, we loop over the points again, and we connect them with all their neihbors. We use
...
func _find_neighbor_indices(cell: Vector2, cell_mappings: Dictionary) -> Array:
	var out := []
	# To find the neighbors, we try to move one cellin every possible direction and ensure that
	...
	for direction in DIRECTIONS:
		...
		# connected. If you don't check for existing connections, you'll get many errors.
