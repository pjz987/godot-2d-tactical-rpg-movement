# Represents a grid with its size, the size of each cell in pixels, and some helper funcitons to
# calculate and convert coordinates.
# It's meant to be shared between game objects that need access to those values.
class_name Grid
extends Resource

# The grid's size in rows and columnts.
...
# Returns the coordinates of the cell on the grid given a postiion on the map.
...
# the grid coordinates they're place on, and call `calculate_map_position()` to snap them to the
# cell's center.
...
# This method and the following on allow us to ensure the cursor or units can never go past the
# map's limit.
...
# w. You can use it for performance.  More onthat below.
